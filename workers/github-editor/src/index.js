...
async function github(url, token, init = {}) { const response = await fetch(`https://api.github.com${url}`, { ...init, headers: { Accept: "application/vnd.github+json", Authorization: `Bearer ${token}`, "X-GitHub-Api-Version": "2022-11-28", ...(init.headers || {}) } }); if (!response.ok) { let detail = ""; try { const payload = await response.json(); detail = String(payload?.message || ""); } catch { } const scopes = response.headers.get("x-oauth-scopes") || ""; throw new Error(`GitHub API ${response.status}${detail ? `: ${detail}` : ""}${scopes ? ` (scopes: ${scopes})` : ""}`); } return response; }
function sessionId(request) { const bearer = request.headers.get("authorization") || ""; return cookie(request, "qimiao_editor_session") || (bearer.startsWith("Bearer ") ? bearer.slice(7).trim() : ""); }
async function session(request, env) { const id = sessionId(request); return id ? await env.SESSIONS.get(`session:${id}`) : null; }
async function clearSession(request, env) { const id = sessionId(request); if (id) await env.SESSIONS.delete(`session:${id}`); }
...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if (url.pathname === "/api/collect-artwork" && request.method === "POST") { const token = await session(request, env); if (!token) return cors(request, env, json({ error: "请先登录 GitHub。" }, { status: 401 })); const requestedAt = new Date().toISOString(); try { await github(`/repos/${env.REPOSITORY}/actions/workflows/collect.yml/dispatches`, token, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ref: env.BRANCH, inputs: { replace_artwork_candidates: "true" } }) }); } catch (error) { const message = String(error?.message || error); if (message.includes("401") || message.includes("403")) { await clearSession(request, env); return cors(request, env, json({ error: "GitHub 授权会话已失效，已清除；请重新登录。" }, { status: 401 })); } throw error; } return cors(request, env, json({ ok: true, requestedAt })); }
