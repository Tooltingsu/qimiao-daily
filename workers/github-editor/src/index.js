...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if (url.pathname === "/api/collect-artwork" && request.method === "POST") { const token = await session(request, env); if (!token) return cors(request, env, json({ error: "请先登录 GitHub。" }, { status: 401 })); const requestedAt = new Date().toISOString(); try { await github(`/repos/${env.REPOSITORY}/actions/workflows/collect.yml/dispatches`, token, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ref: env.BRANCH, inputs: { replace_artwork_candidates: "true" } }) }); } catch (error) { const message = String(error?.message || error); if (message.includes("401")) return cors(request, env, json({ error: "GitHub 授权已失效，请重新登录。" }, { status: 401 })); if (message.includes("403")) return cors(request, env, json({ error: "当前 GitHub OAuth 授权缺少 Actions 权限，需要撤销旧授权后重新授权。" }, { status: 403 })); throw error; } return cors(request, env, json({ ok: true, requestedAt })); }
