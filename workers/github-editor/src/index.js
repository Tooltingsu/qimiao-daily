...
export default { async fetch(request, env) {
  ...
  try {
    ...
    if (url.pathname === "/api/collect-artwork" && request.method === "POST") { const token = await session(request, env); if (!token) return cors(request, env, json({ error: "请先登录 GitHub。" }, { status: 401 })); const requestedAt = new Date().toISOString(); await github(`/repos/${env.REPOSITORY}/actions/workflows/collect.yml/dispatches`, token, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ref: env.BRANCH, inputs: { replace_artwork_candidates: "true" } }) }); return cors(request, env, json({ ok: true, requestedAt })); }
