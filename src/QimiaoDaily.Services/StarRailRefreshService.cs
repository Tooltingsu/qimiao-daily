...
public sealed class StarRailRefreshService(QimiaoDailyDbContext database, StarRailAnnouncementProvider provider)
{
    public async Task<int> RefreshAsync(CancellationToken cancellationToken = default, string? itemType = null)
    {
        ...
        foreach (var candidate in candidates.GroupBy(TimelineChangeClassifier.Identity, StringComparer.OrdinalIgnoreCase).Select(x => x.First()))
        {
            ...
            var result = await importer.ApplyCandidateAsync(candidate, "starrail-announcement-v2", "starrail-provider", "official refresh", cancellationToken);
